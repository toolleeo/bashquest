...
Everytime a challenge is set using `goto`, the setup of the challenge itself (directories, files, etc.) can be different - typically randonmly generated.
The `--seed` option can be used to set a seed for the random number generator, so that the same setup is generated by the challenges.
